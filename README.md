...
- Project phase: implementation — nearing MVP completion.
...
supabase/migrations/20260725200000_audit_log_hash_chain.sql
supabase/migrations/20260729121109_template_content_v3_expand.sql
supabase/migrations/20260730090000_sprint_9_tasks_reminders.sql
supabase/migrations/20260730110000_sprint_10_sms.sql
supabase/migrations/20260730120000_sprint_11_public_form_links.sql
supabase/migrations/20260730180000_sprint_10_notification_preferences.sql
supabase/migrations/20260803160319_document_upload_authorization_barrier.sql
```
...
The document upload-authorization barrier migration requires a controlled deployment window. Pause the resource-purge cron/workers, apply the migration, drain every older application instance that can refresh a pending document upload, then run `select public.reconcile_document_upload_authorization_barriers(clock_timestamp() + interval '7 days');` from the Supabase SQL Editor. Verify the PostgREST schema-cache reload, deploy the new application version, and only then resume purge workers. Completed purge jobs created before this migration have no remaining database object manifest; reconcile their tombstoned R2 prefixes separately before declaring the rollout complete.
