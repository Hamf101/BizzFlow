-- Reconcile the established target with migrations it records as applied.
--
-- A September 12 schema comparison found the hosted project's functions and
-- triggers behind the migration files although its history lists both source
-- migrations. Every object 20260729122927_template_visibility_enforcement
-- defines was still at its previous version there, so its current content
-- never took effect, and the two later fixes to
-- 20260903115718_organization_roles_and_member_access were missing: role
-- archival must count disabled members, and deleting an organization may
-- cascade past Owner protection. Applied history is never edited, so this
-- migration re-applies the current content verbatim.
--
-- Function, trigger, revoke, and grant statements are idempotent. The copied
-- visibility content also repeats its one data statement: active file rows in
-- hidden or ambiguous file fields move to the existing superseded-file cleanup
-- lifecycle. That is the intended state for those rows and a no-op where the
-- backfill already ran. On September 12 the established target held one
-- submission and no active files. There is no table or column statement.
...
-- Current definitions from 20260903115718_organization_roles_and_member_access.
