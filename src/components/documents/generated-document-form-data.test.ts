...
describe("generated document form data", () => {
  ...
  it("omits hidden conditional fields from signing baselines", () => {
    const content = createBlankTemplateContent()
    content.blocks = [
      {
      ...
      },
      {
        ...
        multiline: false,
        visibleWhen: {
          sourceBlockId: "50000000-0000-4000-8000-000000000020",
          operator: "equals",
          value: "Other"
        }
      }
    ...
    expect(
      getGeneratedDocumentAnswerBaselineFields(content, {
        request_type: "Standard",
        request_type_other: "Stale hidden value"
      })
    ).toEqual([
      { name: "answer-baseline.text.request_type", value: "Standard" }
    ])
    expect(
      getGeneratedDocumentAnswerBaselineFields(content, {
        request_type: "Other",
        request_type_other: "Custom request"
      })
    ).toEqual([
      { name: "answer-baseline.text.request_type", value: "Other" },
      {
        name: "answer-baseline.text.request_type_other",
