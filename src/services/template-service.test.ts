...
describe("template service", () => {
  ...
  it("increments revisions while preserving previously generated snapshots", async () => {
    const tables = createBaseTables()
    const originalContent = createUsableTemplateContent()
    ...
    const revisedContent = createUsableTemplateContent()
  ...
  describe("duplicateDocumentTemplate", () => {
    it("duplicates a published template into a draft copy with a new ID", async () => {
      const tables = createBaseTables()
      const client = new FakeClient(tables)

      const duplicated = await duplicateDocumentTemplate(
        {
          actorUserId: MANAGER_ID,
          organizationId: ORG_ID,
          templateId: TEMPLATE_ID
      ...
      expect(duplicated.id).toBe("30000000-0000-4000-8000-000000000099")
      expect(duplicated.status).toBe("draft")
      expect(duplicated.revision).toBe(1)
      expect(duplicated.title).toBe("Published handbook (Copy)")
      expect(duplicated.content).toBeDefined()
    })

    it("duplicates a draft template into another draft copy", async () => {
      const tables = createBaseTables()
      const client = new FakeClient(tables)
