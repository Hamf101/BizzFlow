 ...
 * @param baseAnswers - Optional stored answers used to resolve a partial patch's visibility.
...
export async function normalizeSubmissionDraftAnswers(
  ...
  answers: unknown,
  baseAnswers: Readonly<SubmissionAnswers> = {}
  ...
  const visibleFieldKeys = collectVisibleFieldKeys(snapshot, {
    ...baseAnswers,
    ...normalizedAnswers
  })

  return Object.fromEntries(
    Object.entries(normalizedAnswers).filter(([fieldKey]): boolean =>
...
export async function validateSubmissionForSubmit(
  ...
  const visibleFieldKeys = collectVisibleFieldKeys(snapshot, normalizedAnswers)

  for (const field of fieldByKey.values()) {
    if (!field.required || !visibleFieldKeys.has(field.fieldKey)) {
      continue
    }
...
function collectVisibleFieldKeys(
  ...
  return new Set(
    getVisibleTemplateBlocks(snapshot, answers)
      .filter(
        (block: TemplateBlock): block is SubmissionFieldBlock =>
          "fieldKey" in block
      )
      .map((block: SubmissionFieldBlock): string => block.fieldKey)
  )
}
