  ...
  return runDocumentOperation(
    ...
    async (): Promise<CreateDocumentUploadUrlResponse> => {
      ...
      if (pendingVersionId) {
        ...
        await registerDocumentVersionUploadAuthorization(
          client,
          {
            organizationId: input.organizationId,
            documentId: document.id,
            ...
            expiresAt: refreshedUrl.expiresAt,
          }
        )
      ...
      const { data: createdVersionId, error: allocationError } =
        await client.rpc("create_pending_document_version", {
          ...
          target_upload_authorization_expires_at: signedUrl.expiresAt,
        })
      ...
      await registerDocumentVersionUploadAuthorization(client, {
        organizationId: input.organizationId,
        documentId: document.id,
        versionId,
        uploadedBy: input.actorUserId,
...
type RegisterDocumentVersionUploadAuthorizationInput = {
...
async function registerDocumentVersionUploadAuthorization(
  client: DocumentServiceClient,
  input: RegisterDocumentVersionUploadAuthorizationInput
): Promise<void> {
  ...
  if (error || registered !== true) {
    throw createVersionMutationError(
      error,
      "Unable to authorize replacement upload."
    )
  }
}
