...
import { pruneHiddenAnswerValues } from "@/services/document-signing/answer-validation"
import {
...
export class FakeClient {
  ...
  async rpc(
    ...
    if (functionName === "merge_generated_document_answers") {
      ...
      answer.values = mergeVisibleDocumentAnswers(
        this.tables,
        args,
...
function mergeVisibleDocumentAnswers(
  tables: FakeTables,
  args: Record<string, unknown>,
  storedValues: FakeRow
): Record<string, unknown> {
  const document = tables.documents.find(
    (row: FakeRow): boolean =>
  ...
  const content = parseTemplateContent(document.template_snapshot)
  const visibleStoredValues = pruneHiddenAnswerValues(content, storedValues)

  return pruneHiddenAnswerValues(content, {
    ...visibleStoredValues,
    ...(args.target_values as FakeRow)
  })
}
