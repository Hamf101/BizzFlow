...
export class FakeSupabaseClient {
  ...
  async rpc(
    ...
    if (functionName === "register_document_version_upload_authorization") {
      ...
      if (
        ...
        version.uploaded_by !== args.target_uploaded_by
      ) {
        return {
          data: false,
          error: Object.assign(
            new Error("Document upload authorization cannot be registered."),
            { code: "P0001" }
          ),
        }
      }
      ...
      const currentExpiry = Date.parse(
        String(version.upload_authorization_expires_at ?? "")
      )
      const requestedExpiry = Date.parse(String(args.target_expires_at))

      if (!Number.isFinite(requestedExpiry)) {
        return {
          data: false,
          error: Object.assign(
            new Error("Upload authorization expiry is invalid."),
            { code: "22023" }
          ),
        }
      }

      if (!Number.isFinite(currentExpiry) || requestedExpiry > currentExpiry) {
        version.upload_authorization_expires_at = new Date(
          requestedExpiry
        ).toISOString()
      }
    ...
    if (functionName === "create_pending_document_version") {
      ...
      this.tables.document_versions.push({
        ...
        upload_authorization_expires_at:
          args.target_upload_authorization_expires_at ??
...
export function createVersionRow(overrides: FakeRow = {}): FakeRow {
  return {
    ...
    uploaded_by: "user-1",
    upload_authorization_expires_at: null,
    created_at: "2026-07-09T11:00:00.000Z",
    updated_at: "2026-07-09T11:00:00.000Z",
    ...overrides,
  }
