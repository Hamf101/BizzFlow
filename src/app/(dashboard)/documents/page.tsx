...
/**
 * Renders workspace lifecycle controls for a single document row.
 *
 * Mirrors {@link FolderLifecycleActions} so a file can be archived, restored,
 * trashed, and permanently purged from the list it appears in rather than only
 * from its detail page.
 *
 * @param props - Document, tenant, and safe workspace return values.
 * @returns Lifecycle controls, or null when the actor may not manage the file.
 */
function DocumentLifecycleActions({
  document,
  organizationId,
  returnFolderId,
  role,
  ...
  return (
    <RoleGuard role={role} action="documents:archive">
      <div className="flex flex-col gap-3">
        <div className="flex flex-wrap gap-2">
          {document.lifecycleState === "active" ? (
...
/**
 * Wraps a document lifecycle submit control with its tenant and return fields.
 *
 * The return values are what send the actor back to the workspace view they
 * acted from instead of the document detail page.
 *
 * @param props - Server action, identifiers, and safe workspace return values.
 * @returns Form carrying the hidden lifecycle fields.
 */
