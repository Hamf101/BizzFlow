...
export default async function SubmissionDetailPage({
  ...
  const [result, peopleResult, taskResult] = await Promise.all([
    ...
    listTasks({
      ...
      .then((tasks: Task[]) => ({ tasks, errorMessage: null as string | null }))
      .catch((error: unknown) => {
        const errorMessage = getPageErrorMessage(
          error,
          "Unable to load follow-up tasks."
        )
        console.warn("submission_tasks_load_failed", {
          organizationId: context.organization.id,
          reason: errorMessage,
          submissionId,
          userId: user.id
        })
        return { tasks: [] as Task[], errorMessage }
      })
  ])
      ...
      {submission.status !== "draft" && (
        <div className="grid items-start gap-4 lg:grid-cols-2">
          <div className="flex flex-col gap-4">
            ...
            />
            <SubmissionTaskPanel
              canAssign={canAssignTask && !peopleResult.errorMessage}
              canCreate={canCreateTask}
              createAction={createTaskAction}
              currentUserId={user.id}
              errorMessage={taskResult.errorMessage}
              members={listInternalTaskMembers(peopleResult.members)}
              submissionId={submission.id}
