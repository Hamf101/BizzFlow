...
export const TASK_REMINDER_CHANNELS = ["email", "sms"] as const

/**
 * Who maintains a reminder.
 *
 * `automatic` reminders are derived from a task's due date and assignee and are
 * re-synced whenever either changes. `manual` reminders belong to the member who
 * scheduled them and are never touched by that sync.
 */
...
/** Origin of one scheduled task reminder. */
export type TaskReminderOrigin = (typeof TASK_REMINDER_ORIGINS)[number]
...
/** Zod contract for a persisted task reminder origin. */
export const taskReminderOriginSchema = z.enum(TASK_REMINDER_ORIGINS)
...
const taskReminderRowShape = {
  ...
  origin: taskReminderOriginSchema.default("manual"),
